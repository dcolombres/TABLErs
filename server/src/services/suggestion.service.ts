...
export class SuggestionService {
   ...
   */
  public static suggestCharts(context: SuggestionContext): ChartConfig[] {
  ...
  private static analyzeColumns(columns: string[], data: any[]) {
  ...
  private static isDateColumn(name: string, values: any[]): boolean {
    ...
    if (values.length > 0) {
      ...
      if (v.match(/^\d{4}-\d{2}-\d{2}/) || v.match(/^\d{2}\/\d{2}\/\d{4}/)) return true;
